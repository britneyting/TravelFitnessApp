...
- (void)addHistorywithStartTime:(NSNumber *)start endTime:(NSNumber *)end andLimit:(int)limit {
    ...
    [self.client historyForChannel:self.hybridChannelName start:nil end:nil limit:100 withCompletion:^(PNHistoryResult *result, PNErrorStatus *status) {
...
- (IBAction)sendMessage:(id)sender {
    ...
    [self.client publish:self.Message toChannel:self.currentUser.username withCompletion:^(PNPublishStatus *status) {
        
        if (!status.isError) {

            // Message successfully published to specified channel.
            self.messageField.text = @"";
            NSLog(@"Successfully published message to my channel!");
        }
        else {
            ...
            /**
             ...
             Request can be resent using: [status retry];
             */
            NSLog(@"Didn't publish message to my channel");
        }
    }];
...
- (void)client:(PubNub *)client didReceiveMessage:(PNMessageResult *)message {
    ...
    if (![message.data.channel isEqualToString:message.data.subscription]) {
        ...
        [self.messages insertObject:messageContent atIndex:0];
        [self.tableView reloadData];
        NSIndexPath *indexPath = [[NSIndexPath alloc] init];
        indexPath = [NSIndexPath indexPathForRow:self.messages.count inSection:0];
...
//    cell.profilePicture.file = message[@"profilePicture"];
